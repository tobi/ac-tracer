--[[
CSP Compatibility Shim for LuaJIT-ImGui
Maps CSP's ui.*, ac.*, render.* APIs to ImGui equivalents

This allows ac-tracer code to run unchanged in the simulator.
]]

local ffi = require("ffi")

--------------------------------------------------------------------------------
-- Global State
--------------------------------------------------------------------------------

_G._mockCarPosition = 0
_G._playbackLap = nil
_G._windowSize = { x = 400, y = 200 }
_G._cursorPos = { x = 0, y = 0 }
_G._pathPoints = {}
_G._fontStack = {}
_G._colorStack = {}

--------------------------------------------------------------------------------
-- Math Extensions (CSP provides these)
--------------------------------------------------------------------------------

math.clamp = math.clamp or function(v, min, max)
    if v < min then return min end
    if v > max then return max end
    return v
end

math.sign = math.sign or function(v)
    if v > 0 then return 1 end
    if v < 0 then return -1 end
    return 0
end

math.lerp = math.lerp or function(a, b, t)
    return a + (b - a) * t
end

math.saturate = math.saturate or function(v)
    return math.clamp(v, 0, 1)
end

math.round = math.round or function(v, decimals)
    local mult = 10 ^ (decimals or 0)
    return math.floor(v * mult + 0.5) / mult
end

os.preciseClock = os.preciseClock or os.clock

--------------------------------------------------------------------------------
-- Bit Operations
--------------------------------------------------------------------------------

bit = bit or {}
if not bit.band then
    bit.band = function(a, b)
        local result = 0
        local bitval = 1
        while a > 0 and b > 0 do
            if a % 2 == 1 and b % 2 == 1 then result = result + bitval end
            bitval = bitval * 2
            a = math.floor(a / 2)
            b = math.floor(b / 2)
        end
        return result
    end
    bit.bor = function(a, b)
        local result = 0
        local bitval = 1
        while a > 0 or b > 0 do
            if a % 2 == 1 or b % 2 == 1 then result = result + bitval end
            bitval = bitval * 2
            a = math.floor(a / 2)
            b = math.floor(b / 2)
        end
        return result
    end
    bit.bxor = function(a, b)
        local result = 0
        local bitval = 1
        while a > 0 or b > 0 do
            if (a % 2 == 1) ~= (b % 2 == 1) then result = result + bitval end
            bitval = bitval * 2
            a = math.floor(a / 2)
            b = math.floor(b / 2)
        end
        return result
    end
    bit.bnot = function(a) return -1 - a end
    bit.lshift = function(a, n) return a * (2 ^ n) end
    bit.rshift = function(a, n) return math.floor(a / (2 ^ n)) end
end

--------------------------------------------------------------------------------
-- Vector Types
--------------------------------------------------------------------------------

local function vec2(x, y)
    return { x = x or 0, y = y or 0 }
end

local function vec3(x, y, z)
    return { x = x or 0, y = y or 0, z = z or 0 }
end

local function vec4(x, y, z, w)
    return { x = x or 0, y = y or 0, z = z or 0, w = w or 0 }
end

local function rgb(r, g, b)
    return { r = r or 0, g = g or 0, b = b or 0 }
end

local function make_rgbm(r, g, b, m)
    return { r = r or 0, g = g or 0, b = b or 0, mult = m or 1 }
end

local rgbm = setmetatable({
    colors = {
        white = make_rgbm(1, 1, 1, 1),
        black = make_rgbm(0, 0, 0, 1),
        red = make_rgbm(1, 0, 0, 1),
        green = make_rgbm(0, 1, 0, 1),
        blue = make_rgbm(0, 0, 1, 1),
        yellow = make_rgbm(1, 1, 0, 1),
        transparent = make_rgbm(0, 0, 0, 0),
    }
}, {
    __call = function(_, r, g, b, m)
        return make_rgbm(r, g, b, m)
    end
})

_G.vec2 = vec2
_G.vec3 = vec3
_G.vec4 = vec4
_G.rgb = rgb
_G.rgbm = rgbm

--------------------------------------------------------------------------------
-- Color Conversion Helper
--------------------------------------------------------------------------------

-- Convert rgbm to ImGui color (packed U32 or 4 floats)
local function rgbmToU32(color, imgui)
    if not color then return 0xFFFFFFFF end
    local r = math.min(1, (color.r or 1) * (color.mult or 1))
    local g = math.min(1, (color.g or 1) * (color.mult or 1))
    local b = math.min(1, (color.b or 1) * (color.mult or 1))
    local a = math.min(1, color.mult or 1)
    if imgui and imgui.GetColorU32 then
        return imgui.GetColorU32(r, g, b, a)
    end
    -- Manual packing: ABGR format
    local ai = math.floor(a * 255)
    local bi = math.floor(b * 255)
    local gi = math.floor(g * 255)
    local ri = math.floor(r * 255)
    return ai * 0x1000000 + bi * 0x10000 + gi * 0x100 + ri
end

local function rgbmToFloats(color)
    if not color then return 1, 1, 1, 1 end
    local r = math.min(1, (color.r or 1) * (color.mult or 1))
    local g = math.min(1, (color.g or 1) * (color.mult or 1))
    local b = math.min(1, (color.b or 1) * (color.mult or 1))
    local a = math.min(1, color.mult or 1)
    return r, g, b, a
end

--------------------------------------------------------------------------------
-- ac.* API
--------------------------------------------------------------------------------

ac = {}

local mockSim = {
    trackLengthM = 5000,
    dt = 1/60,
    isPaused = false,
    isReplayActive = false,
    time = 0,
    gameTime = 0,
    currentSessionIndex = 0,
}

ac.getSim = function()
    return mockSim
end

ac.getCar = function(index)
    local pos = _G._mockCarPosition or 0
    local lapData = _G._playbackLap

    if lapData then
        return {
            idValue = lapData.car or "sim_car",
            speedKmh = lapData:speedAt(pos),
            splinePosition = pos,
            lapCount = 1,
            lapTimeMs = (lapData:timeAt(pos) or 0) * 1000,
            bestLapTimeMs = lapData.time or 90000,
            gas = lapData:throttleAt(pos),
            brake = (lapData:brakeAt(pos) or 0) / 100,
            clutch = lapData:clutchAt and lapData:clutchAt(pos) or 0,
            steer = (0.5 - (lapData:steeringAt(pos) or 0.5)) * 2 * math.pi,
            gear = lapData:gearAt(pos),
            fuel = lapData:fuelAt and lapData:fuelAt(pos) or 50,
            isLapValid = true,
            isLastLapValid = true,
            previousLapTimeMs = 0,
            resetCounter = 0,
            wheelsOutside = 0,
            isInPitlane = false,
            isInPit = false,
            tractionControlInAction = false,
            isEngineLimiterOn = false,
            wheels = {
                [0] = { slip = 0.05, ndSlip = 0.05 },
                [1] = { slip = 0.05, ndSlip = 0.05 },
                [2] = { slip = 0.05, ndSlip = 0.05 },
                [3] = { slip = 0.05, ndSlip = 0.05 },
            },
            acceleration = { x = 0, y = 0, z = 0 },
            brakeBias = 0.55,
            tractionControlMode = 5,
            tractionControl2 = 50,
            id = function(self) return self.idValue end,
        }
    end

    -- Default when no lap loaded
    return {
        idValue = "sim_car",
        speedKmh = 0,
        splinePosition = pos,
        lapCount = 1,
        lapTimeMs = 0,
        bestLapTimeMs = 90000,
        gas = 0,
        brake = 0,
        clutch = 0,
        steer = 0,
        gear = 1,
        fuel = 50,
        isLapValid = true,
        isLastLapValid = true,
        previousLapTimeMs = 0,
        resetCounter = 0,
        wheelsOutside = 0,
        isInPitlane = false,
        isInPit = false,
        tractionControlInAction = false,
        isEngineLimiterOn = false,
        wheels = {
            [0] = { slip = 0.05, ndSlip = 0.05 },
            [1] = { slip = 0.05, ndSlip = 0.05 },
            [2] = { slip = 0.05, ndSlip = 0.05 },
            [3] = { slip = 0.05, ndSlip = 0.05 },
        },
        acceleration = { x = 0, y = 0, z = 0 },
        brakeBias = 0.55,
        tractionControlMode = 5,
        tractionControl2 = 50,
        id = function(self) return self.idValue end,
    }
end

ac.getTrackID = function()
    local lapData = _G._playbackLap
    return lapData and lapData.track or "sim_track"
end

ac.getCarID = function(index)
    local lapData = _G._playbackLap
    return lapData and lapData.car or "sim_car"
end

ac.getCarName = function(index, withYear)
    return "Simulator Car"
end

ac.log = function(msg) print("[AC LOG] " .. tostring(msg)) end
ac.warn = function(msg) print("[AC WARN] " .. tostring(msg)) end
ac.error = function(msg) print("[AC ERROR] " .. tostring(msg)) end
ac.setMessage = function(title, message) print(string.format("[AC MSG] %s: %s", title, message)) end

ac.setClipboardText = function(text)
    -- Would need platform-specific implementation
    print("[Clipboard] " .. tostring(text):sub(1, 50))
end

ac.isCarResetAllowed = function() return true end
ac.saveCarStateAsync = function(callback) if callback then callback(nil, { mock = "state" }) end end
ac.loadCarState = function() end

ac.onCarJumped = function(index, callback) end

ac.trackCoordinateToWorld = function(vec)
    return vec3(vec.x * 10, vec.y, vec.z * mockSim.trackLengthM)
end

ac.getTrackSectorName = function(pos) return "Sector" end

ac.readMemoryMappedFile = function() return nil end

ac.ControlButton = function(name, defaults)
    return {
        pressed = function() return false end,
        down = function() return false end,
        control = function(self, size) end,
    }
end

ac.accessAppWindow = function(name)
    return {
        valid = function() return true end,
        visible = function() return true end,
    }
end

ac.AudioEvent = {
    fromFile = function(opts)
        return {
            isValid = function() return false end,
            start = function() end,
            stop = function() end,
            dispose = function() end,
            volume = 1.0,
            pitch = 1.0,
        }
    end
}

-- Storage (in-memory)
local storageData = {}
ac.storage = setmetatable({}, {
    __index = function(t, k) return storageData[k] end,
    __newindex = function(t, k, v) storageData[k] = v end,
    __call = function(t, defaults, prefix)
        local data = {}
        for k, v in pairs(defaults) do
            local storageKey = prefix and (prefix .. k) or k
            data[k] = storageData[storageKey]
            if data[k] == nil then data[k] = v end
        end
        return setmetatable(data, {
            __newindex = function(self, k, v)
                rawset(self, k, v)
                local storageKey = prefix and (prefix .. k) or k
                storageData[storageKey] = v
            end
        })
    end
})

--------------------------------------------------------------------------------
-- ui.* API
--
-- These functions need to be called WITHIN an ImGui frame.
-- They rely on the global `imgui` being available from main.lua
--------------------------------------------------------------------------------

ui = {}

-- Enums (matching CSP values)
ui.Font = { Main = 1, Monospace = 2, Title = 3, Small = 4 }
ui.StyleColor = { Text = 1, Button = 2, ButtonHovered = 3, FrameBg = 4 }
ui.StyleVar = { FramePadding = 1, ItemSpacing = 2 }
ui.InputTextFlags = { None = 0 }
ui.MouseButton = { Left = 1, Right = 2, Middle = 3 }

ui.DWriteFont = function(name)
    return { weight = function(self, w) return self end }
end

-- Layout functions
ui.availableSpace = function()
    if imgui and imgui.GetContentRegionAvail then
        local size = imgui.GetContentRegionAvail()
        return { x = size.x, y = size.y }
    end
    return _G._windowSize
end

ui.availableSpaceX = function()
    local space = ui.availableSpace()
    return space.x
end

ui.setCursor = function(pos)
    if imgui and imgui.SetCursorPos then
        imgui.SetCursorPos(imgui.ImVec2(pos.x, pos.y))
    end
    _G._cursorPos = { x = pos.x or 0, y = pos.y or 0 }
end

ui.getCursor = function()
    if imgui and imgui.GetCursorPos then
        local pos = imgui.GetCursorPos()
        return { x = pos.x, y = pos.y }
    end
    return _G._cursorPos
end

ui.offsetCursorY = function(amount)
    local cur = ui.getCursor()
    ui.setCursor({ x = cur.x, y = cur.y + amount })
end

ui.sameLine = function(x)
    if imgui and imgui.SameLine then
        imgui.SameLine(x or 0)
    end
end

ui.setNextItemWidth = function(width)
    if imgui and imgui.SetNextItemWidth then
        imgui.SetNextItemWidth(width)
    end
end

ui.windowPos = function()
    if imgui and imgui.GetWindowPos then
        local pos = imgui.GetWindowPos()
        return { x = pos.x, y = pos.y }
    end
    return vec2(0, 0)
end

ui.measureText = function(text)
    if imgui and imgui.CalcTextSize then
        local size = imgui.CalcTextSize(text)
        return { x = size.x, y = size.y }
    end
    return vec2(#text * 8, 14)
end

-- Font/Style stack
ui.pushFont = function(font) table.insert(_G._fontStack, font) end
ui.popFont = function() table.remove(_G._fontStack) end
ui.pushStyleColor = function(style, color) table.insert(_G._colorStack, { style = style, color = color }) end
ui.popStyleColor = function(count) for i = 1, (count or 1) do table.remove(_G._colorStack) end end
ui.pushStyleVar = function(var, value) end
ui.popStyleVar = function(count) end
ui.pushItemWidth = function(width) if imgui then imgui.PushItemWidth(width) end end
ui.popItemWidth = function() if imgui then imgui.PopItemWidth() end end

-- Text
ui.text = function(text)
    if imgui and imgui.Text then
        imgui.Text(tostring(text))
    end
end

ui.textColored = function(text, color)
    if imgui and imgui.TextColored then
        local r, g, b, a = rgbmToFloats(color)
        imgui.TextColored(imgui.ImVec4(r, g, b, a), tostring(text))
    end
end

ui.textAligned = function(text, align, size)
    -- ImGui doesn't have direct text alignment, use workaround
    if imgui then
        local textSize = imgui.CalcTextSize(tostring(text))
        local w = size and size.x or ui.availableSpaceX()
        local alignX = align and align.x or 0

        local offsetX = (w - textSize.x) * alignX
        if offsetX > 0 then
            imgui.SetCursorPosX(imgui.GetCursorPosX() + offsetX)
        end
        imgui.Text(tostring(text))
    end
end

ui.dwriteDrawTextClipped = function(text, fontSize, topLeft, bottomRight, alignX, alignY, rtl, color)
    -- Simplified: just draw text at position
    if imgui then
        local draw_list = imgui.GetWindowDrawList()
        local winPos = imgui.GetWindowPos()
        draw_list:AddText(
            imgui.ImVec2(winPos.x + topLeft.x, winPos.y + topLeft.y),
            rgbmToU32(color, imgui),
            tostring(text)
        )
    end
end

-- Drawing primitives (use ImGui draw list)
ui.drawLine = function(p1, p2, color, thickness)
    if imgui then
        local draw_list = imgui.GetWindowDrawList()
        local winPos = imgui.GetWindowPos()
        draw_list:AddLine(
            imgui.ImVec2(winPos.x + p1.x, winPos.y + p1.y),
            imgui.ImVec2(winPos.x + p2.x, winPos.y + p2.y),
            rgbmToU32(color, imgui),
            thickness or 1
        )
    end
end

ui.drawRect = function(p1, p2, color, cornerRadius, thickness)
    if imgui then
        local draw_list = imgui.GetWindowDrawList()
        local winPos = imgui.GetWindowPos()
        draw_list:AddRect(
            imgui.ImVec2(winPos.x + p1.x, winPos.y + p1.y),
            imgui.ImVec2(winPos.x + p2.x, winPos.y + p2.y),
            rgbmToU32(color, imgui),
            cornerRadius or 0,
            0,  -- flags
            thickness or 1
        )
    end
end

ui.drawRectFilled = function(p1, p2, color, cornerRadius)
    if imgui then
        local draw_list = imgui.GetWindowDrawList()
        local winPos = imgui.GetWindowPos()
        draw_list:AddRectFilled(
            imgui.ImVec2(winPos.x + p1.x, winPos.y + p1.y),
            imgui.ImVec2(winPos.x + p2.x, winPos.y + p2.y),
            rgbmToU32(color, imgui),
            cornerRadius or 0
        )
    end
end

ui.drawCircle = function(center, radius, color, segments)
    if imgui then
        local draw_list = imgui.GetWindowDrawList()
        local winPos = imgui.GetWindowPos()
        draw_list:AddCircle(
            imgui.ImVec2(winPos.x + center.x, winPos.y + center.y),
            radius,
            rgbmToU32(color, imgui),
            segments or 0,
            1
        )
    end
end

ui.drawCircleFilled = function(center, radius, color, segments)
    if imgui then
        local draw_list = imgui.GetWindowDrawList()
        local winPos = imgui.GetWindowPos()
        draw_list:AddCircleFilled(
            imgui.ImVec2(winPos.x + center.x, winPos.y + center.y),
            radius,
            rgbmToU32(color, imgui),
            segments or 0
        )
    end
end

ui.drawTriangleFilled = function(p1, p2, p3, color)
    if imgui then
        local draw_list = imgui.GetWindowDrawList()
        local winPos = imgui.GetWindowPos()
        draw_list:AddTriangleFilled(
            imgui.ImVec2(winPos.x + p1.x, winPos.y + p1.y),
            imgui.ImVec2(winPos.x + p2.x, winPos.y + p2.y),
            imgui.ImVec2(winPos.x + p3.x, winPos.y + p3.y),
            rgbmToU32(color, imgui)
        )
    end
end

ui.drawQuadFilled = function(p1, p2, p3, p4, color)
    if imgui then
        local draw_list = imgui.GetWindowDrawList()
        local winPos = imgui.GetWindowPos()
        draw_list:AddQuadFilled(
            imgui.ImVec2(winPos.x + p1.x, winPos.y + p1.y),
            imgui.ImVec2(winPos.x + p2.x, winPos.y + p2.y),
            imgui.ImVec2(winPos.x + p3.x, winPos.y + p3.y),
            imgui.ImVec2(winPos.x + p4.x, winPos.y + p4.y),
            rgbmToU32(color, imgui)
        )
    end
end

-- Path drawing
ui.pathClear = function()
    _G._pathPoints = {}
end

ui.pathLineTo = function(point)
    table.insert(_G._pathPoints, point)
end

ui.pathArcTo = function(center, radius, angleStart, angleEnd, segments)
    segments = segments or 16
    local step = (angleEnd - angleStart) / segments
    for i = 0, segments do
        local angle = angleStart + step * i
        table.insert(_G._pathPoints, {
            x = center.x + math.cos(angle) * radius,
            y = center.y + math.sin(angle) * radius
        })
    end
end

ui.pathStroke = function(color, closed, thickness)
    if imgui and #_G._pathPoints >= 2 then
        local draw_list = imgui.GetWindowDrawList()
        local winPos = imgui.GetWindowPos()

        for i = 1, #_G._pathPoints - 1 do
            local p1 = _G._pathPoints[i]
            local p2 = _G._pathPoints[i + 1]
            draw_list:AddLine(
                imgui.ImVec2(winPos.x + p1.x, winPos.y + p1.y),
                imgui.ImVec2(winPos.x + p2.x, winPos.y + p2.y),
                rgbmToU32(color, imgui),
                thickness or 1
            )
        end

        if closed and #_G._pathPoints >= 2 then
            local p1 = _G._pathPoints[#_G._pathPoints]
            local p2 = _G._pathPoints[1]
            draw_list:AddLine(
                imgui.ImVec2(winPos.x + p1.x, winPos.y + p1.y),
                imgui.ImVec2(winPos.x + p2.x, winPos.y + p2.y),
                rgbmToU32(color, imgui),
                thickness or 1
            )
        end
    end
    _G._pathPoints = {}
end

ui.pathFillConvex = function(color)
    if imgui and #_G._pathPoints >= 3 then
        local draw_list = imgui.GetWindowDrawList()
        local winPos = imgui.GetWindowPos()
        local col = rgbmToU32(color, imgui)

        -- ImGui's PathFillConvex or use triangles
        draw_list:PathClear()
        for _, p in ipairs(_G._pathPoints) do
            draw_list:PathLineTo(imgui.ImVec2(winPos.x + p.x, winPos.y + p.y))
        end
        draw_list:PathFillConvex(col)
    end
    _G._pathPoints = {}
end

-- Controls (basic implementations)
ui.button = function(label, size)
    if imgui and imgui.Button then
        local sz = size and imgui.ImVec2(size.x, size.y) or nil
        return imgui.Button(label, sz)
    end
    return false
end

ui.checkbox = function(label, isChecked)
    if imgui and imgui.Checkbox then
        local arr = ffi.new("bool[1]", isChecked)
        imgui.Checkbox(label, arr)
        return arr[0]
    end
    return isChecked
end

ui.radioButton = function(label, isSelected)
    if imgui and imgui.RadioButton then
        return imgui.RadioButton(label, isSelected)
    end
    return isSelected
end

ui.slider = function(id, value, min, max, format)
    if imgui and imgui.SliderFloat then
        local arr = ffi.new("float[1]", value)
        imgui.SliderFloat(id, arr, min, max, format or "%.2f")
        return arr[0]
    end
    return value
end

ui.inputText = function(id, text, flags)
    if imgui and imgui.InputText then
        local buf = ffi.new("char[256]")
        ffi.copy(buf, text or "")
        local changed = imgui.InputText(id, buf, 256, flags or 0)
        return ffi.string(buf), changed
    end
    return text, false
end

ui.header = function(label)
    if imgui and imgui.CollapsingHeader then
        return imgui.CollapsingHeader(label)
    end
    return true
end

ui.childWindow = function(id, size, fn)
    if imgui and imgui.BeginChild then
        local sz = size and imgui.ImVec2(size.x, size.y) or imgui.ImVec2(0, 0)
        if imgui.BeginChild(id, sz) then
            if fn then fn() end
        end
        imgui.EndChild()
    elseif fn then
        fn()
    end
end

ui.dummy = function(size)
    if imgui and imgui.Dummy then
        imgui.Dummy(imgui.ImVec2(size.x, size.y))
    end
end

ui.invisibleButton = function(id, size)
    if imgui and imgui.InvisibleButton then
        return imgui.InvisibleButton(id, imgui.ImVec2(size.x, size.y))
    end
    return false
end

ui.setTooltip = function(text)
    if imgui and imgui.SetTooltip then
        imgui.SetTooltip(text)
    end
end

-- Mouse input
ui.mouseClicked = function(button)
    if imgui and imgui.IsMouseClicked then
        return imgui.IsMouseClicked(button or 0)
    end
    return false
end

ui.mouseDown = function(button)
    if imgui and imgui.IsMouseDown then
        return imgui.IsMouseDown(button or 0)
    end
    return false
end

ui.mousePos = function()
    if imgui and imgui.GetMousePos then
        local pos = imgui.GetMousePos()
        return { x = pos.x, y = pos.y }
    end
    return vec2(0, 0)
end

ui.mouseWheel = function()
    if imgui and imgui.GetIO then
        return imgui.GetIO().MouseWheel
    end
    return 0
end

ui.rectHovered = function(p1, p2)
    local mouse = ui.mousePos()
    local winPos = ui.windowPos()
    local mx = mouse.x - winPos.x
    local my = mouse.y - winPos.y
    return mx >= p1.x and mx <= p2.x and my >= p1.y and my <= p2.y
end

ui.itemHovered = function()
    if imgui and imgui.IsItemHovered then
        return imgui.IsItemHovered()
    end
    return false
end

--------------------------------------------------------------------------------
-- render.* API
--------------------------------------------------------------------------------

render = {}

-- Debug line (3D) - no-op in 2D simulator
render.debugLine = function(p1, p2, color1, color2)
    -- Would need 3D viewport to implement
end

--------------------------------------------------------------------------------
-- physics.* API
--------------------------------------------------------------------------------

physics = {}

--------------------------------------------------------------------------------
-- stringify
--------------------------------------------------------------------------------

local function serializeValue(v, seen)
    local t = type(v)
    if t == "nil" then return "nil"
    elseif t == "boolean" then return v and "true" or "false"
    elseif t == "number" then return tostring(v)
    elseif t == "string" then return string.format("%q", v)
    elseif t == "table" then
        if seen[v] then return "nil" end
        seen[v] = true
        local parts = {}
        local isArray = true
        local maxIdx = 0
        for k, _ in pairs(v) do
            if type(k) ~= "number" or k < 1 or k ~= math.floor(k) then
                isArray = false
            else
                maxIdx = math.max(maxIdx, k)
            end
        end
        if isArray and maxIdx == #v then
            for i = 1, #v do
                table.insert(parts, serializeValue(v[i], seen))
            end
        else
            for k, val in pairs(v) do
                local keyStr
                if type(k) == "string" and k:match("^[%a_][%w_]*$") then
                    keyStr = k
                else
                    keyStr = "[" .. serializeValue(k, seen) .. "]"
                end
                table.insert(parts, keyStr .. "=" .. serializeValue(val, seen))
            end
        end
        return "{" .. table.concat(parts, ",") .. "}"
    else
        return "nil"
    end
end

stringify = setmetatable({
    binary = function(data) return serializeValue(data, {}) end,
    parse = function(str)
        if not str or str == "" then return nil end
        local fn = load("return " .. str)
        if fn then
            local ok, result = pcall(fn)
            if ok then return result end
        end
        return nil
    end,
    tryParse = function(str, default)
        return stringify.parse(str) or default
    end
}, {
    __call = function(t, data)
        return serializeValue(data, {})
    end
})

--------------------------------------------------------------------------------
-- setTimeout
--------------------------------------------------------------------------------

setTimeout = function(fn, delay)
    if type(fn) == "function" then fn() end
end

--------------------------------------------------------------------------------
-- __dirname
--------------------------------------------------------------------------------

__dirname = "."

--------------------------------------------------------------------------------
-- io extensions
--------------------------------------------------------------------------------

io.createDir = function(path)
    -- Platform-specific
    os.execute('mkdir -p "' .. path .. '"')
    return true
end

io.dirExists = function(path)
    local f = io.open(path, "r")
    if f then f:close() return true end
    return false
end

io.fileExists = function(path)
    local f = io.open(path, "r")
    if f then f:close() return true end
    return false
end

io.scanDir = function(path, pattern)
    -- Would need platform-specific implementation
    return nil
end

io.fileSize = function(path)
    local f = io.open(path, "rb")
    if f then
        local size = f:seek("end")
        f:close()
        return size
    end
    return -1
end

--------------------------------------------------------------------------------
-- extended-brake module mock
--------------------------------------------------------------------------------

package.loaded['extended-brake'] = {
    isAvailable = function() return false end,
    getBrakePressureBar = function(car) return (car and car.brake or 0) * 100, false end,
    getFrontRearBar = function(car) local bar = (car and car.brake or 0) * 100; return bar, bar, false end,
    getBrakeData = function(car) return { value = (car and car.brake or 0) * 100, isFromDLL = false, unit = "bar" } end,
    getBrakePressure = function(car) return (car and car.brake or 0) * 100, false end,
    getFrontRearPressure = function(car) local bar = (car and car.brake or 0) * 100; return bar, bar, false end,
    getNormalizedBrake = function(car) return car and car.brake or 0 end,
    getNormalizedFrontRear = function(car) local b = car and car.brake or 0; return b, b end,
    getStatus = function() return { available = false, source = "sim", status = "simulation mode" } end,
}

print("CSP Compatibility Shim loaded")
